...
// Copyright (c) 2013  The E-Money developers
...
static CBigNum bnProofOfStakeNewLimit(~uint256(0) >> 20);
...
const string strMessageMagic = "E-money Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
   ...
   else if (pindexBest->nHeight+1 >= 6200)
    {
      nSubsidy = 0 * COIN;
      return nSubsidy + nFees;
    }
...
static const int64 nTargetTimespan = 5 * 60; // 5min
static const int64 nTargetNewTimespan = 15 * nStakeTargetSpacing; // 15 * (1 * 60) 15min
static const int64 nTargetSpacingWorkMax = 12 * nStakeTargetSpacing; // 12 * (2 * 60) 24 Minutes
static const int64 nTargetNewSpacingWorkMax = 3 * nStakeTargetSpacing; // 3 * (2 * 60) 6 Minutes
...
{
    if (nTime > 1404997564 ) //Timestamp of block 24800
        return ComputeMaxBits(bnProofOfStakeNewLimit, nBase, nTime);
    else
    ...
    if(fProofOfStake)
       {
           ...
           {
               if(pindexLast->nHeight + 1 > PROTO_CHANGE_BLOCK)
                   bnTargetLimit = bnProofOfStakeNewLimit;
               else if(pindexLast->nHeight + 1 > 100)
                   bnTargetLimit = bnProofOfStakeLimit;
               else if(pindexLast->nHeight + 1 > 90)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in emoney
    bool fStrictPayToScriptHash = true; // Always active in emoney
    ...
    //  fees are not collected by miners as in emoney
...
bool CBlock::AcceptBlock()
{
    ...
    //Pure PoS check
    if ((IsProofOfWork() && nHeight > POW_CUTOFF_BLOCK) || (IsProofOfWork() && fTestNet && nHeight > 1500 ))
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "emoney", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Obama meets boy who inspired $1 million fundraiser";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:emoney genesis block";
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake && GetTime() < nextAllowedBlock) {
            Sleep(1000);
            continue;
        }        
        ...
        if (fProofOfStake)
        {
            ...
            if (pblock->IsProofOfStake())
            {
                ...
                SetThreadPriority(THREAD_PRIORITY_LOWEST);
                // We don't want to flood the network with a metric ton of PoS blocks and fork or get a bunch of orphans, so we send them every 1-2 minutes maximum.
